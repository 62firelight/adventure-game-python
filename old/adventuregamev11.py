...
# (char = character)
player_char = {  # All of these keys/variables are saved
	'player_race' : ' ',
	'player_class' : ' ',
	'player_health' : 0, 
...
#boughtHelmet = False
#boughtArmor = False
# Boolean global variables that are used to determine whether the player has 
# already bought one of each item
...
Your hitpoints: {}""".format(enemy_name, player_xp_gain,
					...
					player_char['player_level'] == player_xp_required
...
#	global boughtWeapon # Declaring global variables for usage later on
#	global boughtShield # so the game 'remembers' if these items have already
#	global boughtBoots  # been bought or not
#	global boughtGauntlets
#	global boughtArmor
...
#		player_char['first_visit_s'] = False
	...
	starter_tier_stats = [2, 6, 1, 2, 3, 4]
	mid_tier_stats = [4, 8, 3, 4, 5, 6]
	end_tier_stats = [6, 10, 6, 8, 10, 12]
...
"What can I do you for?" he asks.""")
	# Store the shop menu's string inside a variable so the whole thing 
...
""".format(player_char['player_gold'])).lower()
		...
		# If the player doesn't have enough gold and hasn't bought the item
		elif (shop_input >= str(1) and shop_input <= str(6)
		and player_char['player_gold'] < prices[int(shop_input) - 1]) and ( 
		player_char['bought_weapon'] == False 
		or player_char['bought_boots'] == False
		or player_char['bought_gauntlets'] == False
		or player_char['bought_helmet'] == False
		or player_char['bought_armor'] == False) :
			print("\nYou don't have enough gold to buy this item!")
		# Buying items from the shop
		elif (shop_input == '1' and player_char['bought_weapon'] == False and 
		player_char['player_gold'] >= prices[0]): # This will always be a weapon
		...
		elif (shop_input == '2' and player_char['bought_shield'] == False and
		player_char['player_gold'] >= prices[1]):
			...
			player_char['player_gold'] -= prices[1]
		elif (shop_input == '3' and player_char['bought_boots'] == False and
		player_char['player_gold'] >= prices[2]):  
			...
			player_char['player_gold'] -= prices[2]
		elif (shop_input == '4' and player_char['bought_gauntlets'] == False and
		player_char['player_gold'] >= prices[3]):
			...
			player_char['player_gold'] -= prices[3]
		elif (shop_input == '5' and player_char['bought_helmet'] == False and
		player_char['player_gold'] >= prices[4]):
			...
			player_char['player_gold'] -= prices[4]
		elif (shop_input == '6' and player_char['boughtArmor'] == False and
		player_char['player_gold'] >= prices[5]):
			...
			player_char['player_gold'] -= prices[5]
		# If the player has already bought the item
#		elif (shop_input >= str(1) and shop_input <= str(6)) and (
#		player_char['bought_weapon'] == True 
#		or player_char['bought_boots'] == True
#		or player_char['bought_gauntlets'] == True
#		or player_char['bought_armor'] == True):
#			print("\nYou have already bought this item!")
#		elif (shop_input >= str(1) and shop_input <= str(6) 
...
#		elif (shop_input >= str(1) and shop_input <= str(6)) and (
#		boughtWeapon == True or boughtBoots == True or
#		boughtGauntlets == True or boughtHelmet == True or
#		boughtArmor == True):
#			print("\nYou have already bought this item!")
#		elif (shop_input >= str(1) and shop_input <= str(6)
#		and player_char['player_gold'] < prices[int(shop_input)]):
#			print("\nYou don't have enough gold to buy this item!")
		elif shop_input == 'r':
			print(shop_menu)
		elif shop_input == "e":
...
[H]eal - "I'm hurt. Can you heal me?" (30 Gold)
...
""").lower()
			...
			if (player_char['player_gold'] >= 30 and 
			player_char['player_health'] < player_char['player_max_hp']):
				player_char['player_health'] = player_char['player_max_hp']
				print("\nHealed back up to {} hitpoints!"
					.format(player_char['player_max_hp']))
				player_char['player_gold'] -= 30
			elif (player_char['player_gold'] >= 30 and
			player_char['player_health'] == player_char['player_max_hp']):
				print("\n\"You don't look hurt at all.\" the innkeeper says.")
			elif (player_char['player_gold'] < 30 and player_char['player_health'] > 5):
				print("\n\"It doesn't look like you've got enough gold.\"")
			elif (player_char['player_gold'] < 30 and player_char['player_health'] < 6):
				print("""
...
Attack : {} """.format(player_char['player_name'], player_char['player_race'],
			 ...
			 player_char['player_max_hp'], player_char['player_defence'],
			 player_char['player_attack']))
