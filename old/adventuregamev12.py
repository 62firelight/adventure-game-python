...
""".format(player_char['player_health'], 
		...
		elif fight_input == 'a' and in_forest == False: # Feeling (out of forest)
...
You hit the {} for {} damage.""".format(enemy_name, fight_damage_p))
				...
				# Gap to separate player and enemy calculations

				fight_damage_e = (random.randint(0, int(enemy_attack)) # Same as before, 
					) - player_char['player_defence']				   # but for enemy damage
...
Your hitpoints: {}""".format(enemy_name, player_xp_gain,
					...
					and player_char['player_level'] < milestones[2]):
						...
						player_char['player_max_hp'] += 2 # Add more health
...
You have grown stronger, but you will need to keep growing if you have 
any desire to survive an encounter with the Red Dragon. """)
							player_char['player_level'] += 1
							player_char['player_max_hp'] += 2
							...
							debug() # placeholder for end of game dialogue
							confirm = input("\nHey\n")
						confirm = input("\nPush any key to continue.\n")
						TrainerLocation()
					break
...
""".format(player_char['player_gold'])).lower()
		...
		player_char['player_gold'] >= prices[4]):
			...
			player_char['player_gold'] -= prices[4]
		elif (shop_input == '6' and player_char['bought_armor'] == False and
...
[R]efill - "Can you refill my healing potions?"
...
of course. I can also refill your own healing potions for free." """)
			confirm = input("\nPush any key to continue.\n") # This should be 
...
on me." """)
			...
			if (player_char['current_healing_potions'] ==
...
Attack : {} """.format(player_char['player_name'], player_char['player_level'], 
			 player_xp_required - player_char['player_experience'], 
...
def StartMenu(): # A lot of the conventions throughout the game are used here:
	...
	# and so starting consecutive new games caused first encounter dialogue
	# for the NPCs to bug out and think that the player had already visited them
	# when they had not.
